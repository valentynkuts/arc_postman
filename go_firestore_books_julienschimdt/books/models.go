...
func MyDocs() {
	...
	states := config.Client.Collection("books")
	ny := states.Doc("MyDocs")

	docsnap, err := ny.Get(context.Background())
	if err != nil {
		// TODO: Handle error.
	}
...
func AddJsonBookWithID(id string, bk Book) error{
...
func GetBookWithID(id string) (Book, error) {
	book := Book{}
	ID:= id
	fmt.Print(ID)
	if ID == "" {
...
func UpdateJsonBookWithID(id string, bk Book) error{
...
func DeleteBookWithID(id string)  error {
	ID:= id
	fmt.Print(ID)
	if ID == "" {
